...
export function MediaForm({ onSubmit, initialValues, billingItems = [], discountRate = 0, onCancel, isSubmitting = false }: MediaFormProps) {
  const initialFormValues: MediaFormValues = {
    ...
    laminating: initialValues?.laminating ?? "no",
  ...
  const [values, setValues] = useState<MediaFormValues>(initialFormValues);
  ...
  const validateForm = (): boolean => {
    ...
    // Laminating validation
    if (!values.laminating || values.laminating === "") {
      newErrors.laminating = "Laminating selection is required";
    }
    ...
    <div className="max-w-3xl w-full mx-auto">
      <div className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          {/* Laminating */}
          <div>
            <label className={labelBase} htmlFor="laminating">
              Laminating <span className="text-red-500">*</span>
            </label>
            <select
              id="laminating"
              name="laminating"
              className={`w-full rounded-lg border ${
                errors.laminating ? "border-red-500" : "border-gray-300"
              } bg-gray-50 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500`}
              value={values.laminating}
              onChange={handleChange}
            >
              <option value="no">No Laminating</option>
              <option value="cool">Cool Laminating</option>
              <option value="hot">Hot Laminating</option>
            </select>
            {errors.laminating && (
              <p className="mt-1 text-xs text-red-600">{errors.laminating}</p>
            )}
          </div>
